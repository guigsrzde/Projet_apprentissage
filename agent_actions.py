...
def draw_graphs_model(csv_path: str):
    ...
    fig, ax = plt.subplots(nrows, ncols,figsize=(35, 25),squeeze=False,gridspec_kw={'hspace': 0.6, 'wspace': 0.4})
    ...
    for i in range(maxturns - 1):
        new_value = mutation_points[-1]
        if i!=0:
            new_value+=2
        for action in actions[i]:
            new_value -= symptom_costs.get(action, 0)
        mutation_points.append(new_value)

    ax[row][col].plot(mutation_points, label='mutation points')
